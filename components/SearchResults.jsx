...
export const SearchResults = ({playlists, songs, artists, setView, setGlobalPlaylistId, setGlobalCurrentSongId, setGlobalIsTrackPlaying, setGlobalArtistId}) => {
        ...
        <div className='space-y-4'>
          ...
          <div className='h-64 pr-8 '>
            <div onClick={()=>selectPlaylist(playlists[0])} className='cursor-pointer relative group h-64 w-full bg-neutral-800 hover:bg-neutral-700 p-4 flex flex-col gap-6 rounded-md transition duration-500'>
              ...
              {/** want to pick the first playlists in the returned playlist array to display in top result card */}
              {playlists && <>
                <img className='h-28 w-28 rounded' src={playlists[0].images[0].url}/>
                <p className='text-3xl font-bold'>{playlists[0].name}</p>
                <p className='text-sm text-neutral-400'>By {playlists[0].owner.display_name}<span className='rounded-full bg-neutral-900 text-white font-bold ml-4 py-1 px-4'>Playlists</span></p>
              </>}
            </div>
          </div>
        </div>
        <div className='space-y-4 '>
          ...
          <div className='flex flex-col'>
            {songs.slice(0,4).map((song)=>{
              {/**h-16 for each 4 songs will make the top songs equal height of top results (h-64) */}
        ...
        <div className='flex flex-wrap gap-4'>
          {artists.slice(0,4).map((artist) => {
            return <div onClick={()=>selectArtist(artist)} key={artist.id} className='cursor-pointer relative group w-56 mb-2 bg-neutral-800 hover:bg-neutral-600 rounded-md p-4'>
              ...
              { artist?.images[0]?.url && <img className='w-48 h-48 mb-4 rounded-full' src={artist.images[0].url}/>}
              <p className='text-base text-white mb-1 w-48 truncate'>{artist.name}</p>
              <p className='text-sm text-neutral-400 mb-8 w-48'>Artist</p>
            </div>
          })}
        </div>
      </div>
